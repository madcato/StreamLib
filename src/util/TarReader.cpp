...
{
using namespace io;
TarReader::TarReader(IInputStream* input)
{
	m_input = input;
}
...
{
	auto_array_ptr<unsigned char> buffer(blockSize,new unsigned char[blockSize]);
	...
	if(tarHeader.typeflag == 'L') // "././@LongLink"
	{
		...
		std::auto_ptr<IInputStream>& pathNameStream = std::auto_ptr<IInputStream>(new TarReaderStream(size,m_input));

		auto_array_ptr<unsigned char> pathNameData(size+1,new unsigned char[size+1]);
	...
	else
	{
		...
		data = std::auto_ptr<IInputStream>(new TarReaderStream(size,m_input));
	}
