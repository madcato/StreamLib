...
public:
	...
	void traceOn(const std::string& fileName, const std::string& appInfo,const long logLevel,const eOpenType& openType);
	void trace(const long severity,const std::string& logLine,bool setTime = true);
	void trace2(const long severity,const std::string& logLine);
	void traceOff();
...
#define LLOG(X,Y) LiteLog::getInstance()->trace(X,Y)
#define LLOG2(X,Y) LiteLog::getInstance()->trace2(X,Y)
} // namespace util
