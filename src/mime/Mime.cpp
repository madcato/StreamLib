...
// store a field to std::string buffer
...
// load a field from std::string buffer
...
bool CMimeField::FindParameter(const char* pszAttr, int& nPos, int& nSize) const
{
	...
	while (pszParms != NULL)
	{
		...
		if (*pszParms == '"')		// quoted std::string
			pszParmEnd = ::strchr(pszParms+1, '"');
		if (!pszParmEnd)			// non quoted std::string
		{
			pszParmEnd = pszParms;
...
// return the length needed to store this header to std::string buffer
...
// store the header to std::string buffer
...
// load a header from std::string buffer
...
// create a new child body part, and add it to body part std::list
...
// return a std::list of all child body parts belong to this body part
...
// return a std::list of all attachment body parts belong to this body part
...
// return the length needed to store this body part to std::string buffer
...
// store the body part to std::string buffer
...
io::Blob<char> CMimeBody::Store() const
{
	int nSize = GetLength();
    ...
    int nSize2 = Store(pBuff, nSize);	
	blob.setData(util::auto_array_ptr<char>(nSize,pBuff),nSize2);

	return blob;
}

// load a body part from std::string buffer
...
// load a field from std::string buffer
int CMimeBody::Load(const io::Blob<char>& data)
{
	return Load(data.get(),data.length());
} 
