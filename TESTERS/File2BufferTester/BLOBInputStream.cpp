...
BLOBInputStream::BLOBInputStream(Blob<char>& blob):m_blob(blob)
{
	m_actualPos = 0;
}
...
{
	return m_blob.size() - m_actualPos;
}
//Closes this input stream and releases any system resources associated with the stream. 
void BLOBInputStream::close()
{
	m_actualPos = m_blob.size();
}
...
{
	long availableLen = m_blob.size() - m_actualPos;
