// BarCodePrint.cpp : Defines the entry point for the console application.
//
      ...
      StartPage(printDlg.hDC);
		 ...
		 for(int i = 0 ; i < codebar.size() ; i++)
		 {
			 ...
			 for(int s = 0 ; s < str.length() ; s++)
			 {
				...
				{
					int c = str[s];
					FillRgn(printDlg.hDC,CreateRectRgn(x, y,  x+ c * anchoUnidad, y +altura),brush);
					x += c * anchoUnidad;
					black = false;
				}
				else
		...
		// Y ahora el texto
		 y += altura +10;
		 x = 20;
		 TextOut(printDlg.hDC,x,y,str.c_str(),str.length());
