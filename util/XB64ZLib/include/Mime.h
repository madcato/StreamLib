...
protected:
	std::list<CMimeField> m_listFields;	// std::list of all header fields
...
public:
	...
	// serialization
	virtual int Store(char* pszData, int nMaxSize) const;
	virtual io::Blob<char> Store() const;
	virtual int Load(const char* pszData, int nDataSize);
	virtual int Load(const io::Blob<char>& data);

protected:
	unsigned char* m_pbText;		// content (text) of the body part
	int m_nTextSize;				// length of content
	CBodyList m_listBodies;			// std::list of all child body parts
